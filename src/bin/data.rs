...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    db_client.simple_query("DROP TABLE IF EXISTS ma_close;").await?;
    db_client.simple_query("DROP TABLE IF EXISTS ma_emat;").await?;
    db_client.simple_query("DROP TABLE IF EXISTS ma_ematw;").await?;
    db_client.simple_query("DROP TABLE IF EXISTS ma_rsi;").await?;
    db_client.simple_query("DROP TABLE IF EXISTS ma_macd;").await?;
    ...
    let (r1, close) = cbbot::candles("BTC-USD", 60, Main).await?;
    let (r2, mut ema12) = cbbot::ema(close, 12).await?;

    let (r3, close) = cbbot::candles("BTC-USD", 60, Main).await?;
    let (r4, mut ema26) = cbbot::ema(close, 26).await?;


    let (r5, mut rsi) = cbbot::rsi("BTC-USD", 60, Main).await?;
    let (r6, mut macd) = cbbot::macd("BTC-USD", 60, Main).await?;
    
    let (r7, mut close) = cbbot::candles("BTC-USD", 60, Main).await?;
    tokio::spawn(async move {
