use cbpro::{
    ...
    websocket::{Channels, WebSocketFeed, MAIN_FEED_URL}
};
...
pub async fn candles(product_id: &str, granularity: i64) -> cbpro::error::Result<(JoinHandle<()>, impl Stream<Item=Candle> + Unpin + Send + 'static)> {

    let client = PublicClient::new(MAIN_URL);
    ...
    let mut feed = WebSocketFeed::connect(MAIN_FEED_URL).await?;
...
pub async fn ema<T>(mut candles: T, window_size: usize) -> cbpro::error::Result<(JoinHandle<()>, impl Stream<Item=Option<(DateTime<Utc>, f64)>> + Unpin + Send + 'static)>
where 
    T: Stream<Item=Candle> + Unpin + Send + 'static,
{
    ...
    let handle = tokio::spawn(async move {
        ...
        while let Some((time, .., close, _, _)) = candles.next().await {
            if count == window_size - 1 {
                ...
                if let Some(ema) = prev_ema {
                    ...
                    if let Err(_) = tx.send(Some((time, ema))).await {
                        println!("receiver dropped");
                        return;
                    }
                } else {
...
pub async fn rsi(product_id: &str, granularity: i64) -> cbpro::error::Result<(JoinHandle<()>, impl Stream<Item=Option<(DateTime<Utc>, f64)>> + Unpin + Send + 'static)> {
    let (mut tx, rx) = mpsc::channel(100);

    let size: usize = 14;
    let (_, gains) = candles(product_id, granularity).await?;
    let gains = gains.map(|mut x|if (x.4 - x.1).is_sign_positive() {x} else {x.4 = 0.; x});
    let (_, mut gains) = ema(gains, size).await?;

    let (_, losses) = candles(product_id, granularity).await?;
...
pub async fn macd(product_id: &str, granularity: i64) -> cbpro::error::Result<(JoinHandle<()>, impl Stream<Item=Option<(DateTime<Utc>, f64)>> + Unpin + Send + 'static)> {
    let (mut tx, rx) = mpsc::channel(100);

    let (_, ema12) = candles(product_id, granularity).await?;
    let (_, mut ema12) = ema(ema12, 12).await?;

    let (_, ema26) = candles(product_id, granularity).await?;
